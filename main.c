...
while(1){
	...
	while(buttonPressed){
	if ((GPIOA->IDR & S4)&&(GPIOB->IDR & S5)&&(GPIOB->IDR & S3)&&(GPIOB->IDR & S2)&&(GPIOB->IDR & S1)&&(GPIOA->IDR & S6)){
	 		  stop();
	 		  endMaze();//need to make
	 }
	else if ((GPIOA->IDR & S5)&&(GPIOB->IDR & S3)&&(GPIOB->IDR & S1)){ //4 way
		stop();
		turnLeft(); //need to make
		doneTurning = false;
		//delay
	}
	else if ((GPIOA->IDR & S5)&&(GPIOB->IDR & S3)){ // -|
		  stop();
		  turnLeft(); //need to make
		  doneTurning = false;
		  //delay
	}
	else if ((GPIOB->IDR & S1)&&(GPIOB->IDR & S3)){ // |-
			straight();
		  // go straight
	}
	else if ((GPIOA->IDR & S5)&&(GPIOB->IDR & S1)){ //T
		  stop();
		  turnLeft(); //need to make
		  doneTurning = false;
		  //delay
	}
	else if ((GPIOA->IDR & S5)){ // <-
		  stop();
		  turnLeft(); //need to make
		  doneTurning = false;
		  //delay
	}
 	else if ((GPIOB->IDR & S1)){ // ->
	  stop();
	  turnRight(); //lock rotation - need to make
	  doneTurning = false;
  	}
	else if (GPIOB->IDR & S2){ //Adjust left
		LeftAdjust();
		doneTurning = false;
		//delay
	}
	else if (GPIOB->IDR & S4){ //Adjust right
			RightAdjust();
			doneTurning = false;
			//delay
		}
  	else if ((GPIOB->IDR & S3)){ //if only s3 high
  		straight();
  		//go straight motor
  	}
